...
// Show_RepairOrders_Table_Mobile.aspx page.  The Row or RecordControl classes are the 
...
namespace OEMConversion.UI.Controls.Show_RepairOrders_Table_Mobile
{
...
// Base class for the RepairOrdersTableControlRow control on the Show_RepairOrders_Table_Mobile page.
...
// Base class for the RepairOrdersTableControl control on the Show_RepairOrders_Table_Mobile page.
...
public class BaseRepairOrdersTableControl : OEMConversion.UI.BaseApplicationTableControl
{
    ...
    // Setup default pagination settings.
      ...
      cols.Add(RepairOrdersTable.RONo, true);
      ...
      foreach(BaseColumn col in cols)
      {
        ...
        protected virtual WhereClause CreateQueryClause()
        {
            ...
            CompoundFilter filter1 = new CompoundFilter(CompoundFilter.CompoundingOperators.And_Operator, null);
            ...
            if (EvaluateFormula("28", false) != "")filter1.AddFilter(new BaseClasses.Data.ColumnValueFilter(BaseClasses.Data.BaseTable.CreateInstance(@"OEMConversion.Business.RepairOrdersTable, App_Code").TableDefinition.ColumnList.GetByUniqueName(@"RepairOrders_.ROSubcontracted"), EvaluateFormula("28", false), BaseClasses.Data.BaseFilter.ComparisonOperator.Not_Equals, false));
            if (EvaluateFormula("52", false) != "")filter1.AddFilter(new BaseClasses.Data.ColumnValueFilter(BaseClasses.Data.BaseTable.CreateInstance(@"OEMConversion.Business.RepairOrdersTable, App_Code").TableDefinition.ColumnList.GetByUniqueName(@"RepairOrders_.ROStatus"), EvaluateFormula("52", false), BaseClasses.Data.BaseFilter.ComparisonOperator.Not_Equals, false));
         ...
         if (EvaluateFormula("52", false) == "--PLEASE_SELECT--" || EvaluateFormula("52", false) == "--ANY--") whereClause.RunQuery = false;
            ...
            whereClause.AddFilter(filter, CompoundFilter.CompoundingOperators.Or_Operator);
    
            whereClause1.AddFilter(filter1, CompoundFilter.CompoundingOperators.And_Operator);
    
            whereClause.iOR(whereClause1);
        ...
        public virtual string[] GetAutoCompletionList_SearchText(String prefixText,int count)
        {
            ...
            foreach (RepairOrdersRecord rec in recordList ){
                ...
                resultItem = rec.Format(RepairOrdersTable.RONo);
  
                if (resultItem != null) {
                    string prText = prefixText;
                    if(RepairOrdersTable.RONo.IsFullTextSearchable) {
                        FullTextExpression ft = new FullTextExpression();
                        prText = ft.GetFirstNonExcludedTerm(prText);
                    }
                    if (!string.IsNullOrEmpty(prText) && resultItem.ToUpper(System.Threading.Thread.CurrentThread.CurrentCulture).Contains(prText.ToUpper(System.Threading.Thread.CurrentThread.CurrentCulture))) {
                        bool isAdded = FormatSuggestions(prText, resultItem, 20, "InMiddleOfMatchedString", "WordsStartingWithSearchString", "[^a-zA-Z0-9]", resultList, RepairOrdersTable.RONo.IsFullTextSearchable);
                        if (isAdded) {
                            continue;
                        }
                    }
...
#region "Helper Functions"
        ...
        public virtual RepairOrdersTableControlRow[] GetSelectedRecordControls()
        {
            ...
            return (RepairOrdersTableControlRow[])(selectedList.ToArray(Type.GetType("OEMConversion.UI.Controls.Show_RepairOrders_Table_Mobile.RepairOrdersTableControlRow")));
