...
// Show_Loaner_request.aspx page.  The Row or RecordControl classes are the 
...
namespace OEMConversion.UI.Controls.Show_Loaner_request
{
...
// Base class for the Loaner_requestRecordControl control on the Show_Loaner_request page.
...
public class BaseLoaner_requestRecordControl : OEMConversion.UI.BaseApplicationRecordControl
{
        ...
        public virtual void Setloaner_inventory_id()
        {
            ...
            // Set the loaner_inventory_id Literal on the webpage with value from the
            // DatabaseAYS_OEM%dbo.loaner_request database record.

            // this.DataSource is the DatabaseAYS_OEM%dbo.loaner_request record retrieved from the database.
            // this.loaner_inventory_id is the ASP:Literal on the webpage.
                  
            if (this.DataSource != null && this.DataSource.IsCreated) {
                								
                // If the loaner_inventory_id is non-NULL, then format the value.
                // The Format method will return the Display Foreign Key As (DFKA) value
               string formattedValue = "";
               Boolean _isExpandableNonCompositeForeignKey = Loaner_requestTable.Instance.TableDefinition.IsExpandableNonCompositeForeignKey(Loaner_requestTable.loaner_inventory_id);
               if(_isExpandableNonCompositeForeignKey &&Loaner_requestTable.loaner_inventory_id.IsApplyDisplayAs)
                                  
                     formattedValue = Loaner_requestTable.GetDFKA(this.DataSource.loaner_inventory_id.ToString(),Loaner_requestTable.loaner_inventory_id, null);
                                    
               if ((!_isExpandableNonCompositeForeignKey) || (String.IsNullOrEmpty(formattedValue)))
                     formattedValue = this.DataSource.Format(Loaner_requestTable.loaner_inventory_id);
                                  
                                
                formattedValue = HttpUtility.HtmlEncode(formattedValue);
                this.loaner_inventory_id.Text = formattedValue;
                   
            } 
            
            else {
            
                // loaner_inventory_id is NULL in the database, so use the Default Value.  
                // Default Value could also be NULL.
        
              this.loaner_inventory_id.Text = EvaluateFormula("URL(\"loaner_inventory_id\")", this.DataSource);
        ...
        public virtual void Setloaner_status_id()
        {
            ...
            if (this.DataSource != null && this.DataSource.IsCreated) {
              ...
              this.loaner_status_id.Text = EvaluateFormula("URL(\"loaner_status_id\")", this.DataSource);
        ...
        public virtual void Setloaner_type_id()
        {
            ...
            if (this.DataSource != null && this.DataSource.IsCreated) {
              ...
              this.loaner_type_id.Text = EvaluateFormula("URL(\"loaner_type_id\")", this.DataSource);
      ...
      // To customize, override this method in Loaner_requestRecordControl.
         ...
         
        {
            ...
            HttpContext.Current.Session["QueryString in Show-Loaner-request"] = recId;
...
#region "Helper Properties"
        ...
        public System.Web.UI.WebControls.Literal loaner_inventory_id {
            get {
                return (System.Web.UI.WebControls.Literal)BaseClasses.Utils.MiscUtils.FindControlRecursively(this, "loaner_inventory_id");
            }
        }
