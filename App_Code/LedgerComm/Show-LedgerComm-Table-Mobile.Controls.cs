...
// Show_LedgerComm_Table_Mobile.aspx page.  The Row or RecordControl classes are the 
...
namespace OEMConversion.UI.Controls.Show_LedgerComm_Table_Mobile
{
...
// Base class for the LedgerCommTableControlRow control on the Show_LedgerComm_Table_Mobile page.
...
public class BaseLedgerCommTableControlRow : OEMConversion.UI.BaseApplicationRecordControl
{
        ...
        public virtual void SetLdgSubject()
        {
            ...
            // Set the LdgSubject Label on the webpage with value from the
            // DatabaseAYS_OEM%dbo.LedgerComm database record.

            // this.DataSource is the DatabaseAYS_OEM%dbo.LedgerComm record retrieved from the database.
            // this.LdgSubject is the ASP:Label on the webpage.
      ...
      // To customize, override this method in LedgerCommTableControlRow.
        ...
        public virtual void ViewRowButton_Click(object sender, ImageClickEventArgs args)
        {
            ...
            string url = @"../LedgerComm/Show-LedgerComm-Mobile.aspx?LedgerComm={PK}";
...
#region "Helper Properties"
        ...
        public System.Web.UI.WebControls.Label LdgSubject {
            get {
                return (System.Web.UI.WebControls.Label)BaseClasses.Utils.MiscUtils.FindControlRecursively(this, "LdgSubject");
            }
        }
...
// Base class for the LedgerCommTableControl control on the Show_LedgerComm_Table_Mobile page.
...
public class BaseLedgerCommTableControl : OEMConversion.UI.BaseApplicationTableControl
{
       ...
       public BaseLedgerCommTableControl()
        ...
        protected virtual void Control_Init(object sender, System.EventArgs e)
        {
           ...
           // Setup the filter and search.
            ...
            {
                string initialVal = "";
                if  (this.InSession(this.LdgSubjectFilter)) 				
                    initialVal = this.GetFromSession(this.LdgSubjectFilter);
                
                else
                    
                    initialVal = EvaluateFormula("URL(\"LdgSubject\")");
                
                if (initialVal != null && initialVal != "")		
                {
                        
                    this.LdgSubjectFilter.Items.Add(new ListItem(initialVal, initialVal));
                        
                    this.LdgSubjectFilter.SelectedValue = initialVal;
    ...
    // Setup default pagination settings.
       ...
       // Setup the sorting events.
            ...
            this.LdgSubjectFilter.SelectedIndexChanged += new EventHandler(LdgSubjectFilter_SelectedIndexChanged);
      ...
      public override void DataBind()
      {
        ...
        System.Web.UI.WebControls.Repeater rep1 = (System.Web.UI.WebControls.Repeater)BaseClasses.Utils.MiscUtils.FindControlRecursively(rep.Items[PageIndex], "LedgerCommTableControlRepeater1");
        ...
        public virtual void ResetControl()
        {
            ...
            this.LdgSubjectFilter.ClearSelection();
        ...
        public virtual WhereClause CreateWhereClause()
        {
            ...
            if (MiscUtils.IsValueSelected(this.LdgSubjectFilter)) {
                        
                wc.iAND(LedgerCommTable.LdgSubject, BaseFilter.ComparisonOperator.EqualsTo, MiscUtils.GetSelectedValue(this.LdgSubjectFilter, this.GetFromSession(this.LdgSubjectFilter)), false, false);
      ...
      String LdgSubjectFilterSelectedValue = (String)HttpContext.Current.Session[HttpContext.Current.Session.SessionID + appRelativeVirtualPath + "LdgSubjectFilter_Ajax"];
            if (MiscUtils.IsValueSelected(LdgSubjectFilterSelectedValue)) {

              
                wc.iAND(LedgerCommTable.LdgSubject, BaseFilter.ComparisonOperator.EqualsTo, LdgSubjectFilterSelectedValue, false, false);
      ...
      foreach(BaseColumn col in cols)
      {
            ...
            foreach (LedgerCommRecord rec in recordList ){
                ...
                if (resultItem != null) {
                    ...
                    if (!string.IsNullOrEmpty(prText) && resultItem.ToUpper(System.Threading.Thread.CurrentThread.CurrentCulture).Contains(prText.ToUpper(System.Threading.Thread.CurrentThread.CurrentCulture))) {
                        bool isAdded = FormatSuggestions(prText, resultItem, 20, "InMiddleOfMatchedString", "WordsStartingWithSearchString", "[^a-zA-Z0-9]", resultList, LedgerCommTable.LdgFormType.IsFullTextSearchable);
                        if (isAdded) {
                            continue;
                        }
                    }
        ...
        protected virtual void AddNewRecords()
        {
            ...
            if (!this.ResetData)
            {
              System.Web.UI.WebControls.Repeater rep = (System.Web.UI.WebControls.Repeater)(BaseClasses.Utils.MiscUtils.FindControlRecursively(this, "LedgerCommTableControlRepeater"));
              if (rep == null){return;}
              LedgerCommTableControlRow[] recControls = this.GetRecordControls();
                foreach (LedgerCommTableControlRow recControl in recControls) 
		{
      ...
      LedgerCommRecord rec = new LedgerCommRecord();
        ...
        public virtual void SetLdgSubjectFilter()
        {
            
            this.PopulateLdgSubjectFilter(MiscUtils.GetSelectedValue(this.LdgSubjectFilter,  GetFromSession(this.LdgSubjectFilter)), 500);					
        ...
        // Get the filters' data for LdgSubjectFilter.
                
        protected virtual void PopulateLdgSubjectFilter(string selectedValue, int maxItems)
                    
        {
            ...
            this.LdgSubjectFilter.Items.Clear();
            WhereClause wc = this.CreateWhereClause_LdgSubjectFilter();            
            			  			
            // Set up the WHERE and the ORDER BY clause by calling the CreateWhereClause_LdgSubjectFilter function.
            // It is better to customize the where clause there.
             
            // Setup the static list items        
            
                // Add the All item.
                this.LdgSubjectFilter.Items.Insert(0, new ListItem(this.Page.GetResourceValue("Txt:All", "OEMConversion"), "--ANY--"));
              
            
            
            OrderBy orderBy = new OrderBy(false, false);
            orderBy.Add(LedgerCommTable.LdgSubject, OrderByItem.OrderDir.Asc);                
            ...
            if (wc.RunQuery)
            {
                ...
                values = LedgerCommTable.GetValues(LedgerCommTable.LdgSubject, wc, orderBy, maxItems);
            ...
            string fvalue;
            if ( LedgerCommTable.LdgSubject.IsColumnValueTypeBoolean()) {
                    fvalue = cvalue;
                }else {
                    fvalue = LedgerCommTable.LdgSubject.Format(cvalue);
                }
                if (fvalue == null) {
                    fvalue = "";
                }
                ...
                ListItem dupItem = this.LdgSubjectFilter.Items.FindByText(fvalue);
                ...
                this.LdgSubjectFilter.Items.Add(newItem);
            ...
            try
            {
                ...
                // Set the selected value.
                MiscUtils.SetSelectedValue(this.LdgSubjectFilter, selectedValue);
            ...
            if (this.LdgSubjectFilter.SelectedValue != null && this.LdgSubjectFilter.Items.FindByValue(this.LdgSubjectFilter.SelectedValue) == null)
                this.LdgSubjectFilter.SelectedValue = null;
                           
        }
            
        public virtual WhereClause CreateWhereClause_LdgSubjectFilter()
        {
            // Create a where clause for the filter LdgSubjectFilter.
            // This function is called by the Populate method to load the items 
            // in the LdgSubjectFilterDropDownList
        ...
        protected override void SaveControlsToSession()
        {
            ...
            this.SaveToSession(this.LdgSubjectFilter, this.LdgSubjectFilter.SelectedValue);
      ...
      this.SaveToSession("LdgSubjectFilter_Ajax", this.LdgSubjectFilter.SelectedValue);
      ...
      this.SaveToSession("SearchText_Ajax", this.SearchText.Text);
        ...
        protected override void ClearControlsFromSession()
        {
            ...
            this.RemoveFromSession(this.LdgSubjectFilter);
    ...
    this.ViewState["Page_Size"] = this.PageSize;
      ...
      this.Page.Response.Redirect(url);
        ...
        protected virtual void LdgSubjectFilter_SelectedIndexChanged(object sender, EventArgs args)
        {
            // Setting the DataChanged to True results in the page being refreshed with
            // the most recent data from the database.  This happens in PreRender event
            // based on the current sort, search and filter criteria.
...
#region "Helper Properties"
        ...
        public System.Web.UI.WebControls.DropDownList LdgSubjectFilter {
            get {
                return (System.Web.UI.WebControls.DropDownList)BaseClasses.Utils.MiscUtils.FindControlRecursively(this, "LdgSubjectFilter");
            }
        }
...
#region "Helper Functions"
        ...
        public virtual LedgerCommTableControlRow[] GetSelectedRecordControls()
        {
            ...
            return (LedgerCommTableControlRow[])(selectedList.ToArray(Type.GetType("OEMConversion.UI.Controls.Show_LedgerComm_Table_Mobile.LedgerCommTableControlRow")));
