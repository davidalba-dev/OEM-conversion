...
// Show_Project_Table1.aspx page.  The Row or RecordControl classes are the 
...
namespace OEMConversion.UI.Controls.Show_Project_Table1
{
...
// Base class for the ProjectTableControlRow control on the Show_Project_Table1 page.
...
public class BaseProjectTableControlRow : OEMConversion.UI.BaseApplicationRecordControl
{
        ...
        public virtual void Setproject_actual()
        {
            
                    
            // Set the project_actual Literal on the webpage with value from the
            // DatabaseAYS_OEM%dbo.project database record.

            // this.DataSource is the DatabaseAYS_OEM%dbo.project record retrieved from the database.
            // this.project_actual is the ASP:Literal on the webpage.
                  
            if (this.DataSource != null && this.DataSource.project_actualSpecified) {
                								
                // If the project_actual is non-NULL, then format the value.
                // The Format method will use the Display Format
               string formattedValue = this.DataSource.Format(ProjectTable.project_actual, @"C");
                                
                formattedValue = HttpUtility.HtmlEncode(formattedValue);
                this.project_actual.Text = formattedValue;
                   
            } 
            
            else {
            
                // project_actual is NULL in the database, so use the Default Value.  
                // Default Value could also be NULL.
        
              this.project_actual.Text = ProjectTable.project_actual.Format(ProjectTable.project_actual.DefaultValue, @"C");
            		
            }
            
            // If the project_actual is NULL or blank, then use the value specified  
            // on Properties.
            if (this.project_actual.Text == null ||
                this.project_actual.Text.Trim().Length == 0) {
                // Set the value specified on the Properties.
                this.project_actual.Text = "&nbsp;";
            }
        ...
        public virtual void Getproject_actual()
        {
      ...
      this.Page.Response.Redirect(url);
        ...
        public virtual void createdby_Click(object sender, EventArgs args)
        {
            ...
            string url = @"../Technicians/Show-Technicians.aspx?Technicians={ProjectTableControlRow:FK:FK_project_Technicians}";
...
#region "Helper Properties"
        ...
        public System.Web.UI.WebControls.Literal project_actual {
            get {
                return (System.Web.UI.WebControls.Literal)BaseClasses.Utils.MiscUtils.FindControlRecursively(this, "project_actual");
            }
        }
...
// Base class for the ProjectTableControl control on the Show_Project_Table1 page.
...
public class BaseProjectTableControl : OEMConversion.UI.BaseApplicationTableControl
{
           ...
           // Setup the filter and search.
            ...
            if (!this.Page.IsPostBack)
            {
                ...
                if (initialVal != null && initialVal != "")		
                {
                    ...
                    string[] project_status_idFilteritemListFromSession = initialVal.Split(',');
                    int index = 0;
                    foreach (string item in project_status_idFilteritemListFromSession)
                    {
                        ...
                        {
                            this.project_status_idFilter.Items.Add(item);
                            this.project_status_idFilter.Items[index].Selected = true;
                            index += 1;
                        }
                    }
                    foreach (ListItem listItem in this.project_status_idFilter.Items)
                    {
    ...
    // Setup default pagination settings.
       ...
       // Setup the sorting events.
            ...
            this.SortControl.SelectedIndexChanged += new EventHandler(SortControl_SelectedIndexChanged);
              ...
              this.project_status_idFilter.SelectedIndexChanged += project_status_idFilter_SelectedIndexChanged;                  
    ...
    public virtual void AddWarningMessageOnClick() {
        ...
        public void PreFetchForeignKeyValues() {
            ...
            this.Page.PregetDfkaRecords(ProjectTable.createdby, this.DataSource);
        ...
        public virtual WhereClause CreateWhereClause()
        {
            ...
            if (MiscUtils.IsValueSelected(this.project_status_idFilter)) {
                ...
                foreach (ListItem item in this.project_status_idFilter.Items){
                    if (item.Selected) {
                        selectedItemCount += 1;
                ...
                foreach (ListItem item in this.project_status_idFilter.Items){
                    ...
                    if (item.Selected){
                        filter.iOR(ProjectTable.project_status_id, BaseFilter.ComparisonOperator.EqualsTo, item.Value, false, false);
                    }
                }
                wc.iAND(filter);
      ...
      String project_status_idFilterSelectedValue = (String)HttpContext.Current.Session[HttpContext.Current.Session.SessionID + appRelativeVirtualPath + "project_status_idFilter_Ajax"];
        ...
        if (project_status_idFilterSelectedValue != null){
                        string[] project_status_idFilteritemListFromSession = project_status_idFilterSelectedValue.Split(',');
                        int index = 0;
                        WhereClause filter = new WhereClause();
                        foreach (string item in project_status_idFilteritemListFromSession)
                        {
                            ...
                            {
                                filter.iOR(ProjectTable.project_status_id, BaseFilter.ComparisonOperator.EqualsTo, item, false, false);
                                index += 1;
                            }
                        }
                        wc.iAND(filter);
      ...
      ProjectRecord rec = new ProjectRecord();
                        ...
                        if (recControl.created_datetime.Text != "") {
                            rec.Parse(recControl.created_datetime.Text, ProjectTable.created_datetime);
                  }
                
                        if (recControl.createdby.Text != "") {
                            rec.Parse(recControl.createdby.Text, ProjectTable.createdby);
                  }
                
                        if (recControl.project_actual.Text != "") {
                            rec.Parse(recControl.project_actual.Text, ProjectTable.project_actual);
                  }
        ...
        public virtual void Setproject_status_idFilter()
        {
            ...
            ArrayList project_status_idFilterselectedFilterItemList = new ArrayList();
            string project_status_idFilteritemsString = null;
            if (this.InSession(this.project_status_idFilter))
                project_status_idFilteritemsString = this.GetFromSession(this.project_status_idFilter);
            
            if (project_status_idFilteritemsString != null)
            {
                string[] project_status_idFilteritemListFromSession = project_status_idFilteritemsString.Split(',');
                foreach (string item in project_status_idFilteritemListFromSession)
                {
                    project_status_idFilterselectedFilterItemList.Add(item);
                }
            }
              
            			
            this.Populateproject_status_idFilter(MiscUtils.GetSelectedValueList(this.project_status_idFilter, project_status_idFilterselectedFilterItemList), 500);
                    
              string url = this.ModifyRedirectUrl("../project_status/Project-status-QuickSelector.aspx", "", true);
              
              url = this.Page.ModifyRedirectUrl(url, "", true);                                  
              
              url += "?Target=" + this.project_status_idFilter.ClientID + "&DFKA=" + (this.Page as BaseApplicationPage).Encrypt("project_status")+ "&IndexField=" + (this.Page as BaseApplicationPage).Encrypt("project_status_id")+ "&EmptyValue=" + (this.Page as BaseApplicationPage).Encrypt("--ANY--") + "&EmptyDisplayText=" + (this.Page as BaseApplicationPage).Encrypt(this.Page.GetResourceValue("Txt:All")) + "&RedirectStyle=" + (this.Page as BaseApplicationPage).Encrypt("Popup");
              
              this.project_status_idFilter.Attributes["onClick"] = "initializePopupPage(this, '" + url + "', " + Convert.ToString(project_status_idFilter.AutoPostBack).ToLower() + ", event); return false;";
        ...
        protected virtual void Populateproject_status_idFilter(ArrayList selectedValue, int maxItems)
                    
        {
                    ...
                    foreach (Project_statusRecord itemValue in itemValues) 
                    {
                        ...
                        if (itemValue.project_status_idSpecified) 
                        {
                            ...
                            if (counter < maxItems && this.project_status_idFilter.Items.FindByValue(cvalue) == null)
                            {
                                ...
                                Boolean _isExpandableNonCompositeForeignKey = ProjectTable.Instance.TableDefinition.IsExpandableNonCompositeForeignKey(ProjectTable.project_status_id);
                                if(_isExpandableNonCompositeForeignKey && ProjectTable.project_status_id.IsApplyDisplayAs)
                                     fvalue = ProjectTable.GetDFKA(itemValue, ProjectTable.project_status_id);
                                if ((!_isExpandableNonCompositeForeignKey) || (String.IsNullOrEmpty(fvalue)))
                                     fvalue = itemValue.Format(Project_statusTable.project_status);
            ...
            this.project_status_idFilter.SetFieldMaxLength(50);
                                 
                  
            // Add the selected value.
            if (this.project_status_idFilter.Items.Count == 0)
                this.project_status_idFilter.Items.Add(new ListItem(Page.GetResourceValue("Txt:All", "OEMConversion"), "--ANY--"));
        ...
        public virtual WhereClause CreateWhereClause_project_status_idFilter()
        {
            ...
            // in the project_status_idFilterQuickSelector
            ...
            project_status_idFilterselectedFilterItemList = MiscUtils.GetSelectedValueList(this.project_status_idFilter, project_status_idFilterselectedFilterItemList); 
            WhereClause wc = new WhereClause();
            if (project_status_idFilterselectedFilterItemList == null || project_status_idFilterselectedFilterItemList.Count == 0)
                wc.RunQuery = false;
            else            
            {
                foreach (string item in project_status_idFilterselectedFilterItemList)
                {
            	  
                    wc.iOR(Project_statusTable.project_status_id, BaseFilter.ComparisonOperator.EqualsTo, item);                  
        ...
        protected override void SaveControlsToSession()
        {
            ...
            ArrayList project_status_idFilterselectedFilterItemList = MiscUtils.GetSelectedValueList(this.project_status_idFilter, null);
            string project_status_idFilterSessionString = "";
            if (project_status_idFilterselectedFilterItemList != null){
                foreach (string item in project_status_idFilterselectedFilterItemList){
                    project_status_idFilterSessionString = String.Concat(project_status_idFilterSessionString ,"," , item);
                }
            }
            this.SaveToSession(this.project_status_idFilter, project_status_idFilterSessionString);
        ...
        protected  void SaveControlsToSession_Ajax()
        {
            ...
            this.SaveToSession("project_status_idFilter_Ajax", project_status_idFilterSessionString);
    ...
    this.ViewState["Page_Size"] = this.PageSize;
              ...
              
        {
                    ...
                    this.ImportButton.PostBackUrl = "../Shared/SelectFileToImport.aspx?TableName=Project" ;
        ...
        public virtual void ExcelButton_Click(object sender, ImageClickEventArgs args)
        {
            ...
            // Create the Order By clause based on the user's current sorting preference.
             ...
             ProjectTable.project_actual,
             ProjectTable.project_est_hours,
             ProjectTable.createdby,
             ProjectTable.created_datetime,
             null};
             ...
             data.ColumnList.Add(new ExcelColumn(ProjectTable.project_actual, "Currency"));
             data.ColumnList.Add(new ExcelColumn(ProjectTable.project_est_hours, "0"));
             data.ColumnList.Add(new ExcelColumn(ProjectTable.createdby, "Default"));
             data.ColumnList.Add(new ExcelColumn(ProjectTable.created_datetime, "Short Date"));
        ...
        public virtual void PDFButton_Click(object sender, ImageClickEventArgs args)
        {
            ...
            try {
                ...
                report.SpecificReportFileName = Page.Server.MapPath("Show-Project-Table1.PDFButton.report");
                // report.Title replaces the value tag of page header and footer containing ${ReportTitle}
                report.Title = "project";
                // If Show-Project-Table1.PDFButton.report specifies a valid report template,
                ...
                // The 5th parameter represents the relative width of the column
                 ...
                 report.AddColumn(ProjectTable.project_actual.Name, ReportEnum.Align.Right, "${project_actual}", ReportEnum.Align.Right, 20);
                 report.AddColumn(ProjectTable.project_est_hours.Name, ReportEnum.Align.Right, "${project_est_hours}", ReportEnum.Align.Right, 15);
                 report.AddColumn(ProjectTable.createdby.Name, ReportEnum.Align.Left, "${createdby}", ReportEnum.Align.Left, 24);
                 report.AddColumn(ProjectTable.created_datetime.Name, ReportEnum.Align.Left, "${created_datetime}", ReportEnum.Align.Left, 20);
                     ...
                     if (records != null && records.Length > 0 && whereClause.RunQuery)
                    {
                    ...
                    
                        {
                            ...
                            // The 4th parameter represent the maximum length of the data value being shown
                             ...
                             report.AddData("${project_actual}", record.Format(ProjectTable.project_actual), ReportEnum.Align.Right, 300);
                             ...
                             if (BaseClasses.Utils.MiscUtils.IsNull(record.createdby)){
                                 report.AddData("${createdby}", "",ReportEnum.Align.Left);
                             }else{
                                 Boolean _isExpandableNonCompositeForeignKey;
                                 String _DFKA = "";
                                 _isExpandableNonCompositeForeignKey = ProjectTable.Instance.TableDefinition.IsExpandableNonCompositeForeignKey(ProjectTable.createdby);
                                 _DFKA = ProjectTable.GetDFKA(record.createdby.ToString(), ProjectTable.createdby,null);
                                 if (_isExpandableNonCompositeForeignKey &&  ( _DFKA  != null)  &&  ProjectTable.createdby.IsApplyDisplayAs){
                                     report.AddData("${createdby}", _DFKA,ReportEnum.Align.Left);
                                 }else{
                                     report.AddData("${createdby}", record.Format(ProjectTable.createdby), ReportEnum.Align.Left);
                                 }
                             }
                             report.AddData("${created_datetime}", record.Format(ProjectTable.created_datetime), ReportEnum.Align.Left, 300);
        ...
        public virtual void WordButton_Click(object sender, ImageClickEventArgs args)
        {
            ...
            try {
                ...
                report.SpecificReportFileName = Page.Server.MapPath("Show-Project-Table1.WordButton.word");
                // report.Title replaces the value tag of page header and footer containing ${ReportTitle}
                report.Title = "project";
                // If Show-Project-Table1.WordButton.report specifies a valid report template,
                // AddColumn methods will generate a report template.
                // Each AddColumn method-call specifies a column
                // The 1st parameter represents the text of the column header
                // The 2nd parameter represents the horizontal alignment of the column header
...
#region "Helper Properties"
        ...
        public BaseClasses.Web.UI.WebControls.QuickSelector project_status_idFilter {
            get {
                return (BaseClasses.Web.UI.WebControls.QuickSelector)BaseClasses.Utils.MiscUtils.FindControlRecursively(this, "project_status_idFilter");
            }
        }              
...
#region "Helper Functions"
        ...
        public virtual ProjectTableControlRow[] GetSelectedRecordControls()
        {
            ...
            return (ProjectTableControlRow[])((new ArrayList()).ToArray(Type.GetType("OEMConversion.UI.Controls.Show_Project_Table1.ProjectTableControlRow")));
