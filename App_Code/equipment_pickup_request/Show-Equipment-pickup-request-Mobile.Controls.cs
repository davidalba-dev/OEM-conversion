...
// Show_Equipment_pickup_request_Mobile.aspx page.  The Row or RecordControl classes are the 
...
namespace OEMConversion.UI.Controls.Show_Equipment_pickup_request_Mobile
{
...
// Base class for the Equipment_pickup_requestRecordControl control on the Show_Equipment_pickup_request_Mobile page.
...
public class BaseEquipment_pickup_requestRecordControl : OEMConversion.UI.BaseApplicationRecordControl
{
      ...
      // To customize, override this method in Equipment_pickup_requestRecordControl.
         ...
         
        {
            ...
            HttpContext.Current.Session["QueryString in Show-Equipment-pickup-request-Mobile"] = recId;
        ...
        public virtual WhereClause CreateWhereClause_picked_up_byDropDownList() 
        {
            ...
            // This WhereClause is for the DatabaseAYS_OEM%dbo.equipment_pickup_request table.
            // Examples:
            // wc.iAND(Equipment_pickup_requestTable.technician_id, BaseFilter.ComparisonOperator.EqualsTo, "XYZ");
            // wc.iAND(Equipment_pickup_requestTable.Active, BaseFilter.ComparisonOperator.EqualsTo, "1");
            CompoundFilter filter = new CompoundFilter(CompoundFilter.CompoundingOperators.And_Operator, null);
            WhereClause whereClause = new WhereClause();
            
            filter.AddFilter(new BaseClasses.Data.ColumnJoinFilter(BaseClasses.Data.BaseTable.CreateInstance(@"OEMConversion.Business.Equipment_pickup_requestTable, App_Code").TableDefinition.ColumnList.GetByUniqueName(@"equipment_pickup_request_.technician_id"), new BaseClasses.Data.IdentifierAliasInfo(@"equipment_pickup_request_", null), BaseClasses.Data.BaseTable.CreateInstance(@"OEMConversion.Business.TechniciansTable, App_Code").TableDefinition.ColumnList.GetByUniqueName(@"Technicians_.technician_id"), new BaseClasses.Data.IdentifierAliasInfo(@"Technicians_", null),  BaseClasses.Data.BaseFilter.ComparisonOperator.EqualsTo, false));
        ...
        protected virtual void Populatepicked_up_byDropDownList(string selectedValue, int maxItems) 
        {
            ...
            OrderBy orderBy = new OrderBy(false, false);
                          orderBy.Add(Equipment_pickup_requestTable.picked_up_by, OrderByItem.OrderDir.Asc);
            ...
            Equipment_pickup_requestRecord[] itemValues  = null;
            if (wc.RunQuery)
            {
                int counter = 0;
                int pageNum = 0;	
                ...
                do
                {
                    itemValues = Equipment_pickup_requestTable.GetRecords(wc, orderBy, pageNum, maxItems);
                    foreach (Equipment_pickup_requestRecord itemValue in itemValues) 
                    {
                        // Create the item and add to the list.
                        string cvalue = null;
                        string fvalue = null;
                        if (itemValue.picked_up_bySpecified) 
                        {
                            cvalue = itemValue.picked_up_by.ToString().ToString();
                            if (counter < maxItems && this.picked_up_by.Items.FindByValue(cvalue) == null)
                            {
                                ...
                                fvalue = EvaluateFormula("picked_up_by", itemValue, variables2, evaluator);
                ...
                !MiscUtils.SetSelectedDisplayText(this.picked_up_by, selectedValue))
            {
                ...
                // construct a whereclause to query a record with DatabaseAYS_OEM%dbo.equipment_pickup_request.picked_up_by = selectedValue
                    
                CompoundFilter filter2 = new CompoundFilter(CompoundFilter.CompoundingOperators.And_Operator, null);
                WhereClause whereClause2 = new WhereClause();
                filter2.AddFilter(new BaseClasses.Data.ColumnValueFilter(Equipment_pickup_requestTable.picked_up_by, selectedValue, BaseClasses.Data.BaseFilter.ComparisonOperator.EqualsTo, false));
                whereClause2.AddFilter(filter2, CompoundFilter.CompoundingOperators.And_Operator);

                // Execute the query
                try
                {
                    Equipment_pickup_requestRecord[] rc = Equipment_pickup_requestTable.GetRecords(whereClause2, new OrderBy(false, false), 0, 1);
                    System.Collections.Generic.IDictionary<string, object> vars = new System.Collections.Generic.Dictionary<string, object> ();
                    // if find a record, add it to the dropdown and set it as selected item
                    if (rc != null && rc.Length == 1)
                    {
                        Equipment_pickup_requestRecord itemValue = rc[0];
                        string cvalue = null;
                        string fvalue = null;                        
                        if (itemValue.picked_up_bySpecified)
                            cvalue = itemValue.picked_up_by.ToString(); 
                        ...
                        fvalue = EvaluateFormula("picked_up_by", itemValue, variables2, evaluator2);
