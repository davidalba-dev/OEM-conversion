...
// Edit_Customers_Billing.aspx page.  The Row or RecordControl classes are the 
...
namespace OEMConversion.UI.Controls.Edit_Customers_Billing
{
...
// Base class for the CustomersRecordControl control on the Edit_Customers_Billing page.
...
public class BaseCustomersRecordControl : OEMConversion.UI.BaseApplicationRecordControl
{
        ...
        {      
              ...
              this.cust_billing_province.SelectedIndexChanged += cust_billing_province_SelectedIndexChanged;
        ...
        public virtual void Setcust_billing_province()
        {
            ...
            // Set the cust_billing_province DropDownList on the webpage with value from the
            // DatabaseAYS_OEM%dbo.Customers database record.
            
            // this.DataSource is the DatabaseAYS_OEM%dbo.Customers record retrieved from the database.
            // this.cust_billing_province is the ASP:DropDownList on the webpage.
            
            // You can modify this method directly, or replace it with a call to
            //     base.Setcust_billing_province();
            ...
            if (this.DataSource != null && this.DataSource.cust_billing_provinceSpecified)
            {
                ...
                // The Format method will use the Display Format
                selectedValue = this.DataSource.cust_billing_province;
            ...
            else
            {
                ...
                if (this.DataSource != null && this.DataSource.IsCreated)
                    ...
                    selectedValue = CustomersTable.cust_billing_province.DefaultValue;
            ...
            this.Populatecust_billing_provinceDropDownList(selectedValue, 100);              
        ...
        {
         // Retrieve the value entered by the user on the cust_billing_province ASP:DropDownList, and
            // save it into the cust_billing_province field in DataSource DatabaseAYS_OEM%dbo.Customers record.
            
            // Custom validation should be performed in Validate, not here.
            
            this.DataSource.Parse(MiscUtils.GetValueSelectedPageRequest(this.cust_billing_province), CustomersTable.cust_billing_province);			
      ...
      // To customize, override this method in CustomersRecordControl.
         ...
         
        {
            ...
            HttpContext.Current.Session["QueryString in Edit-Customers-Billing"] = recId;
        ...
        public virtual WhereClause CreateWhereClause_cust_billing_provinceDropDownList() 
        {
        ...
        // Fill the cust_billing_province list.
        protected virtual void Populatecust_billing_provinceDropDownList(string selectedValue, int maxItems) 
        {
            		  					                
            this.cust_billing_province.Items.Clear();
            
            // 1. Setup the static list items        
            
              // Add the Please Select item.
              this.cust_billing_province.Items.Insert(0, new ListItem(this.Page.GetResourceValue("Txt:PleaseSelect", "OEMConversion"), "--PLEASE_SELECT--"));
            		  			
            // 2. Set up the WHERE and the ORDER BY clause by calling the CreateWhereClause_cust_billing_provinceDropDownList function.
            // It is better to customize the where clause there.
            
                      
            WhereClause wc = CreateWhereClause_cust_billing_provinceDropDownList();
            ...
            // 3. Read a total of maxItems from the database and insert them into the cust_billing_provinceDropDownList.
                ...
                do
                {
                    ...
                    foreach (ProvincesRecord itemValue in itemValues) 
                    {
                        ...
                        if (itemValue.ProvCodeSpecified) 
                        {
                            ...
                            if (counter < maxItems && this.cust_billing_province.Items.FindByValue(cvalue) == null)
                            {
                                ...
                                ListItem dupItem = this.cust_billing_province.Items.FindByText(fvalue);
                                ...
                                this.cust_billing_province.Items.Add(newItem);
            ...
            if (selectedValue != null &&
                selectedValue.Trim() != "" &&
                !MiscUtils.SetSelectedValue(this.cust_billing_province, selectedValue) &&
                !MiscUtils.SetSelectedDisplayText(this.cust_billing_province, selectedValue))
            {
                ...
                try
                {
                    ...
                    if (rc != null && rc.Length == 1)
                    {
                        ...
                        MiscUtils.ResetSelectedItem(this.cust_billing_province, new ListItem(fvalue, cvalue));                      
                    }
                }
                catch
                {
        ...
        protected virtual void cust_billing_province_SelectedIndexChanged(object sender, EventArgs args)
        {
            // for the value inserted by quick add button or large list selector, 
            // the value is necessary to be inserted by this event during postback 
            string val = (string)(this.Page.Session[cust_billing_province.ClientID + "_SelectedValue"]);
            string displayText = (string)(this.Page.Session[cust_billing_province.ClientID + "_SelectedDisplayText"]);
            if (!string.IsNullOrEmpty(displayText) && !string.IsNullOrEmpty(val)) {
	            this.cust_billing_province.Items.Add(new ListItem(displayText, val));
	            this.cust_billing_province.SelectedIndex = this.cust_billing_province.Items.Count - 1;
	            this.Page.Session.Remove(cust_billing_province.ClientID + "_SelectedValue");
	            this.Page.Session.Remove(cust_billing_province.ClientID + "_SelectedDisplayText");
            }
...
#region "Helper Properties"
        ...
        public System.Web.UI.WebControls.DropDownList cust_billing_province {
            get {
                return (System.Web.UI.WebControls.DropDownList)BaseClasses.Utils.MiscUtils.FindControlRecursively(this, "cust_billing_province");
            }
        }
...
// Base class for the TechniciansRecordControl control on the Edit_Customers_Billing page.
