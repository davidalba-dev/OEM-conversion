...
// Warehouse_location_QuickSelector.aspx page.  The Row or RecordControl classes are the 
...
namespace OEMConversion.UI.Controls.Warehouse_location_QuickSelector
{
...
// Base class for the SelectorTableControlRow control on the Warehouse_location_QuickSelector page.
             ...
             
        {
            ...
            if (this.DataSource != null) {
              ...
              if (this.RecordUniqueId != null) {
                ...
                return Warehouse_locationTable.GetRecord(this.RecordUniqueId, true);
...
// Base class for the SelectorTableControl control on the Warehouse_location_QuickSelector page.
...
public class BaseSelectorTableControl : OEMConversion.UI.BaseApplicationTableControl
{
    ...
    // Setup default pagination settings.
       ...
       // Setup the sorting events.
        ...
        public virtual void LoadData()
        {
            ...
            try {
                  ...
                  if (wc != null && !wc.RunQuery) {
                      ...
                      Type myrec = typeof(OEMConversion.Business.Warehouse_locationRecord);
                      this.DataSource = (Warehouse_locationRecord[])(alist.ToArray(myrec));
                ...
                if (this.AddNewRecord > 0) {
                    ...
                    Type myrec = typeof(OEMConversion.Business.Warehouse_locationRecord);
                    this.DataSource = (Warehouse_locationRecord[])(postdata.ToArray(myrec));
                } 
                else {
        ...
        public virtual Warehouse_locationRecord[] GetRecords(BaseFilter join, WhereClause where, OrderBy orderBy, int pageIndex, int pageSize)
        {    
            ...
            // selCols.Add(Warehouse_locationTable.Column1, true);          
            // selCols.Add(Warehouse_locationTable.Column2, true);          
            // selCols.Add(Warehouse_locationTable.Column3, true);          
                  ...
                  
            {
                ...
                return Warehouse_locationTable.GetRecords(join, where, orderBy, this.PageIndex, this.PageSize);
                 
            }
            else
            {
                Warehouse_locationTable databaseTable = new Warehouse_locationTable();
                ...
                return (recList.ToArray(typeof(Warehouse_locationRecord)) as Warehouse_locationRecord[]);
            }            
        ...
        public virtual int GetRecordCount(BaseFilter join, WhereClause where)
        {
            ...
            if (selCols.Count == 0 || selCols.HasPrimaryKey())
                ...
                return Warehouse_locationTable.GetRecordCount(join, where);
            else
            {
                Warehouse_locationTable databaseTable = new Warehouse_locationTable();
                databaseTable.SelectedColumns.Clear();
        ...
        {    
            ...
            {
                BaseColumn col = Warehouse_locationTable.GetColumnByName(field);
            ...
            if (includeFormulaAndDFKAParameter) 
            {
                if (formula != "")
                {
                    foreach (BaseColumn col in MiscUtils.GetFormulaFieldList(formula, Warehouse_locationTable.Instance.TableDefinition))
                    {
                        if (col != null && !columnList.Contains(col))
                            columnList.Add(col);             
                    }
                ...
                {
                    BaseColumn col = Warehouse_locationTable.GetColumnByName(displayFieldName);
                    if (col != null && !columnList.Contains(col))
                        columnList.Add(col);             
                }
            }
      ...
      foreach(BaseColumn col in cols)
      {
          ...
          search.iOR(col, BaseFilter.ComparisonOperator.Contains, MiscUtils.GetSelectedValue(this.Search, this.GetFromSession(this.Search)), true, false);
		        ...
		        if (!string.IsNullOrEmpty(this.Page.Request["dependOnField"]) && this.Page.Request["dependOnVal"] != null) {
			          BaseColumn dependOnCol = Warehouse_locationTable.GetColumnByName(this.Page.GetDecryptedURLParameter("dependOnField"));
                if (dependOnCol != null){                
      ...
      foreach(BaseColumn col in cols)
      {
        ...
        public virtual string[] GetAutoCompletionList_Search(String prefixText,int count)
        {
            ...
            if(count==0) count = 10;
            OEMConversion.Business.Warehouse_locationRecord[] recordList  = Warehouse_locationTable.GetRecords(filterJoin, wc, null, 0, count, ref count);
            String resultItem = "";
            if (resultItem == "") resultItem = "";
            foreach (Warehouse_locationRecord rec in recordList ){
      ...
      Warehouse_locationRecord rec = new Warehouse_locationRecord();
            ...
            for (int count = 1; count <= this.AddNewRecord; count++) {
                ...
                newRecordList.Insert(0, new Warehouse_locationRecord());
            ...
            if (newRecordList.Count > 0) {
                ...
                Type myrec = typeof(OEMConversion.Business.Warehouse_locationRecord);
                this.DataSource = (Warehouse_locationRecord[])(finalList.ToArray(myrec));
      ...
      this.Page.Response.Redirect(url);
        ...
        public int TotalRecords 
        {
            get {
                if (_TotalRecords < 0)
                {
                    _TotalRecords = Warehouse_locationTable.GetRecordCount(CreateCompoundJoinFilter(), CreateWhereClause());
                }
        ...
        public  Warehouse_locationRecord[] DataSource {
            ...
            get {
                return (Warehouse_locationRecord[])(base._DataSource);
            }
            set {
                this._DataSource = value;
            }
...
#region "Helper Functions"
        ...
        public virtual SelectorTableControlRow[] GetSelectedRecordControls()
        {
            ...
            return (SelectorTableControlRow[])((new ArrayList()).ToArray(Type.GetType("OEMConversion.UI.Controls.Warehouse_location_QuickSelector.SelectorTableControlRow")));
