...
// Add_Equipment_inventory_labor.aspx page.  The Row or RecordControl classes are the 
...
namespace OEMConversion.UI.Controls.Add_Equipment_inventory_labor
{
...
// Base class for the Equipment_inventory_laborRecordControl control on the Add_Equipment_inventory_labor page.
...
public class BaseEquipment_inventory_laborRecordControl : OEMConversion.UI.BaseApplicationRecordControl
{
        ...
        {      
              ...
              this.technician_id1.SelectedIndexChanged += technician_id1_SelectedIndexChanged;
        ...
        public virtual void LoadData()  
        {
            ...
            this.DataSource = (Equipment_inventory_laborRecord)Equipment_inventory_laborRecord.Copy(recList[0], false);
        ...
        public virtual void Setdate0()
        {
            ...
            if (this.DataSource != null && this.DataSource.IsCreated) {
              ...
              this.date0.Text = EvaluateFormula("Today()", this.DataSource, @"d");
        ...
        public virtual void Setinventory_id()
        {
            ...
            else
            {
                ...
                selectedValue = EvaluateFormula("URL(\"inventory_id\")");
        ...
        public virtual void Settech_role_id()
        {
            ...
            else
            {
                ...
                selectedValue = EvaluateFormula("URL(\"tech_role_id\")");
        ...
        public virtual void Settechnician_id1()
        {
            ...
            // this.DataSource is the DatabaseAYS_OEM%dbo.equipment_inventory_labor record retrieved from the database.
            // this.technician_id1 is the ASP:DropDownList on the webpage.
            
            // You can modify this method directly, or replace it with a call to
            //     base.Settechnician_id1();
            // and add your own custom code before or after the call to the base function.
            ...
            this.Populatetechnician_id1DropDownList(selectedValue, 100);              
        ...
        public virtual void Settechnician_idLabel()
                  {
                      ...
                      //this.technician_idLabel.Text = "Some value";
        ...
        public virtual void Gettechnician_id1()
        {
         // Retrieve the value entered by the user on the technician_id ASP:DropDownList, and
            ...
            this.DataSource.Parse(MiscUtils.GetValueSelectedPageRequest(this.technician_id1), Equipment_inventory_laborTable.technician_id);			
      ...
      // To customize, override this method in Equipment_inventory_laborRecordControl.
         ...
         
        {
            ...
            string recId = this.Page.Request.QueryString["Equipment_inventory_labor"];
            ...
            HttpContext.Current.Session["QueryString in Add-Equipment-inventory-labor"] = recId;
        ...
        public virtual WhereClause CreateWhereClause_inventory_idDropDownList() 
        {
            ...
            if (EvaluateFormula("URL(\"inventory_id\")", false) != "")filter.AddFilter(new BaseClasses.Data.ColumnValueFilter(BaseClasses.Data.BaseTable.CreateInstance(@"OEMConversion.Business.Equipment_inventoryTable, App_Code").TableDefinition.ColumnList.GetByUniqueName(@"equipment_inventory_.inventory_id"), EvaluateFormula("URL(\"inventory_id\")", false), BaseClasses.Data.BaseFilter.ComparisonOperator.EqualsTo, false));
         if (EvaluateFormula("URL(\"inventory_id\")", false) == "--PLEASE_SELECT--" || EvaluateFormula("URL(\"inventory_id\")", false) == "--ANY--") whereClause.RunQuery = false;
        ...
        public virtual WhereClause CreateWhereClause_technician_id1DropDownList() 
        {
            // By default, we simply return a new WhereClause.
            // Add additional where clauses to restrict the items shown in the dropdown list.
                    ...
                    foreach (Equipment_inventoryRecord itemValue in itemValues) 
                    {
                        ...
                        if (itemValue.inventory_idSpecified) 
                        {
                            ...
                            if (counter < maxItems && this.inventory_id.Items.FindByValue(cvalue) == null)
                            {
                                ...
                                fvalue = EvaluateFormula("= Equipment_inventory.serial_number", itemValue, variables2, evaluator);
                ...
                !MiscUtils.SetSelectedDisplayText(this.inventory_id, selectedValue))
            {
                ...
                try
                {
                    ...
                    if (rc != null && rc.Length == 1)
                    {
                        ...
                        fvalue = EvaluateFormula("= Equipment_inventory.serial_number", itemValue, variables2, evaluator2);
        ...
        // Fill the technician_id1 list.
        protected virtual void Populatetechnician_id1DropDownList(string selectedValue, int maxItems) 
        {
            		  					                
            this.technician_id1.Items.Clear();
            
            // 1. Setup the static list items        
            
              // Add the Please Select item.
              this.technician_id1.Items.Insert(0, new ListItem(this.Page.GetResourceValue("Txt:PleaseSelect", "OEMConversion"), "--PLEASE_SELECT--"));
            		  			
            // 2. Set up the WHERE and the ORDER BY clause by calling the CreateWhereClause_technician_id1DropDownList function.
            // It is better to customize the where clause there.
            
                      
            WhereClause wc = CreateWhereClause_technician_id1DropDownList();
            ...
            // 3. Read a total of maxItems from the database and insert them into the technician_id1DropDownList.
            TechniciansRecord[] itemValues  = null;
            if (wc.RunQuery)
            {
                int counter = 0;
                ...
                do
                {
                    ...
                    foreach (TechniciansRecord itemValue in itemValues) 
                    {
                        ...
                        {
                            cvalue = itemValue.technician_id.ToString().ToString();
                            if (counter < maxItems && this.technician_id1.Items.FindByValue(cvalue) == null)
                            {
                                ...
                                ListItem dupItem = this.technician_id1.Items.FindByText(fvalue);
                                ...
                                ListItem newItem = new ListItem(fvalue, cvalue);
                                this.technician_id1.Items.Add(newItem);
            ...
            if (selectedValue != null &&
                selectedValue.Trim() != "" &&
                !MiscUtils.SetSelectedValue(this.technician_id1, selectedValue) &&
                !MiscUtils.SetSelectedDisplayText(this.technician_id1, selectedValue))
            {
                ...
                try
                {
                    ...
                    if (rc != null && rc.Length == 1)
                    {
                        ...
                        if (fvalue == null || fvalue.Trim() == "") fvalue = cvalue;
                        MiscUtils.ResetSelectedItem(this.technician_id1, new ListItem(fvalue, cvalue));                      
                    }
                }
                catch
                {
        ...
        protected virtual void technician_id1_SelectedIndexChanged(object sender, EventArgs args)
        {
            // for the value inserted by quick add button or large list selector, 
            // the value is necessary to be inserted by this event during postback 
            string val = (string)(this.Page.Session[technician_id1.ClientID + "_SelectedValue"]);
            string displayText = (string)(this.Page.Session[technician_id1.ClientID + "_SelectedDisplayText"]);
            if (!string.IsNullOrEmpty(displayText) && !string.IsNullOrEmpty(val)) {
	            this.technician_id1.Items.Add(new ListItem(displayText, val));
	            this.technician_id1.SelectedIndex = this.technician_id1.Items.Count - 1;
	            this.Page.Session.Remove(technician_id1.ClientID + "_SelectedValue");
	            this.Page.Session.Remove(technician_id1.ClientID + "_SelectedDisplayText");
            }
...
#region "Helper Properties"
        ...
        public System.Web.UI.WebControls.DropDownList technician_id1 {
            get {
                return (System.Web.UI.WebControls.DropDownList)BaseClasses.Utils.MiscUtils.FindControlRecursively(this, "technician_id1");
            }
        }
    ...
    Equipment_inventory_laborRecord rec = null;
             ...
             
        {
            ...
            return new Equipment_inventory_laborRecord();
