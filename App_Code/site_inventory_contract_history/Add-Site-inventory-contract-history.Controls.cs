...
// Add_Site_inventory_contract_history.aspx page.  The Row or RecordControl classes are the 
...
namespace OEMConversion.UI.Controls.Add_Site_inventory_contract_history
{
...
//public class Vw_site_inventory_customerTableControl : BaseVw_site_inventory_customerTableControl
//{
//    // The BaseVw_site_inventory_customerTableControl class implements the LoadData, DataBind, CreateWhereClause
//    // and other methods to load and display the data in a table control.
//
//    // This is the ideal place to add your code customizations. You can override the LoadData and CreateWhereClause,
//    // The Vw_site_inventory_customerTableControlRow class offers another place where you can customize
//    // the DataBind, GetUIData, SaveData and Validate methods specific to each row displayed on the table.
//    
//}
//
//public class Vw_site_inventory_customerTableControlRow : BaseVw_site_inventory_customerTableControlRow
//{
//      
//        // The BaseVw_site_inventory_customerTableControlRow implements code for a ROW within the
//        // the Vw_site_inventory_customerTableControl table.  The BaseVw_site_inventory_customerTableControlRow implements the DataBind and SaveData methods.
//        // The loading of data is actually performed by the LoadData method in the base class of Vw_site_inventory_customerTableControl.
//
...
// Base class for the Site_inventory_contract_historyRecordControl control on the Add_Site_inventory_contract_history page.
...
public class BaseSite_inventory_contract_historyRecordControl : OEMConversion.UI.BaseApplicationRecordControl
{
        ...
        public virtual void LoadData()  
        {
            ...
            this.DataSource = (Site_inventory_contract_historyRecord)Site_inventory_contract_historyRecord.Copy(recList[0], false);
        ...
        public virtual void Setsite_inventory_contract_id()
        {
            ...
            // Set the site_inventory_contract_id DropDownList on the webpage with value from the
            // DatabaseAYS_OEM%dbo.site_inventory_contract_history database record.
            
            // this.DataSource is the DatabaseAYS_OEM%dbo.site_inventory_contract_history record retrieved from the database.
            // this.site_inventory_contract_id is the ASP:DropDownList on the webpage.
            ...
            else
            {
                ...
                selectedValue = EvaluateFormula("URL(\"site_inventory_contract_id\")");
            ...
            this.Populatesite_inventory_contract_idDropDownList(selectedValue, 100);              
        ...
        public virtual void Setsite_inventory_id()
        {
            ...
            if (this.DataSource != null && this.DataSource.IsCreated) {
              ...
              this.site_inventory_id.Text = EvaluateFormula("URL(\"site_inventory_id\")", this.DataSource);
        ...
        {
         // Retrieve the value entered by the user on the site_inventory_contract_id ASP:DropDownList, and
        ...
        public virtual void Getsite_inventory_id()
        {
            ...
            // Save the value to data source
            this.DataSource.Parse(this.site_inventory_id.Text, Site_inventory_contract_historyTable.site_inventory_id);							
      ...
      // To customize, override this method in Site_inventory_contract_historyRecordControl.
         ...
         
        {
            ...
            string recId = this.Page.Request.QueryString["Site_inventory_contract_history"];
            ...
            HttpContext.Current.Session["QueryString in Add-Site-inventory-contract-history"] = recId;
        ...
        public virtual WhereClause CreateWhereClause_site_inventory_contract_idDropDownList() 
        {
            // By default, we simply return a new WhereClause.
            // Add additional where clauses to restrict the items shown in the dropdown list.
            						
            // This WhereClause is for the DatabaseAYS_OEM%dbo.site_inventory_contract table.
            // Examples:
            // wc.iAND(Site_inventory_contractTable.contract_number, BaseFilter.ComparisonOperator.EqualsTo, "XYZ");
            // wc.iAND(Site_inventory_contractTable.Active, BaseFilter.ComparisonOperator.EqualsTo, "1");
            CompoundFilter filter = new CompoundFilter(CompoundFilter.CompoundingOperators.And_Operator, null);
            WhereClause whereClause = new WhereClause();
            
            if (EvaluateFormula("URL(\"custno\")", false) != "")filter.AddFilter(new BaseClasses.Data.ColumnValueFilter(BaseClasses.Data.BaseTable.CreateInstance(@"OEMConversion.Business.Site_inventory_contractTable, App_Code").TableDefinition.ColumnList.GetByUniqueName(@"site_inventory_contract_.custno"), EvaluateFormula("URL(\"custno\")", false), BaseClasses.Data.BaseFilter.ComparisonOperator.EqualsTo, false));
            if (EvaluateFormula("1", false) != "")filter.AddFilter(new BaseClasses.Data.ColumnValueFilter(BaseClasses.Data.BaseTable.CreateInstance(@"OEMConversion.Business.Site_inventory_contractTable, App_Code").TableDefinition.ColumnList.GetByUniqueName(@"site_inventory_contract_.contract_active"), EvaluateFormula("1", false), BaseClasses.Data.BaseFilter.ComparisonOperator.EqualsTo, false));
            if (EvaluateFormula("Today()", false) != "")filter.AddFilter(new BaseClasses.Data.ColumnValueFilter(BaseClasses.Data.BaseTable.CreateInstance(@"OEMConversion.Business.Site_inventory_contractTable, App_Code").TableDefinition.ColumnList.GetByUniqueName(@"site_inventory_contract_.end_date"), EvaluateFormula("Today()", false), BaseClasses.Data.BaseFilter.ComparisonOperator.Greater_Than, false));
        ...
        // Fill the site_inventory_contract_id list.
        protected virtual void Populatesite_inventory_contract_idDropDownList(string selectedValue, int maxItems) 
        {
            		  					                
            this.site_inventory_contract_id.Items.Clear();
            
            // 1. Setup the static list items        
            
              // Add the Please Select item.
              this.site_inventory_contract_id.Items.Insert(0, new ListItem(this.Page.GetResourceValue("Txt:PleaseSelect", "OEMConversion"), "--PLEASE_SELECT--"));
            		  			
            // 2. Set up the WHERE and the ORDER BY clause by calling the CreateWhereClause_site_inventory_contract_idDropDownList function.
            // It is better to customize the where clause there.
            
                      
            WhereClause wc = CreateWhereClause_site_inventory_contract_idDropDownList();
                        
                
            // Create the ORDER BY clause to sort based on the displayed value.							
                
            OrderBy orderBy = new OrderBy(false, false);
                          orderBy.Add(Site_inventory_contractTable.contract_number, OrderByItem.OrderDir.Asc);

            System.Collections.Generic.IDictionary<string, object> variables = new System.Collections.Generic.Dictionary<string, object> ();
            FormulaEvaluator evaluator = new FormulaEvaluator();

            // 3. Read a total of maxItems from the database and insert them into the site_inventory_contract_idDropDownList.
            Site_inventory_contractRecord[] itemValues  = null;
            if (wc.RunQuery)
            {
                int counter = 0;
                int pageNum = 0;	
                ...
                do
                {
                    itemValues = Site_inventory_contractTable.GetRecords(wc, orderBy, pageNum, maxItems);
                    foreach (Site_inventory_contractRecord itemValue in itemValues) 
                    {
                        // Create the item and add to the list.
                        string cvalue = null;
                        string fvalue = null;
                        ...
                        {
                            cvalue = itemValue.site_inventory_contract_id.ToString().ToString();
                            if (counter < maxItems && this.site_inventory_contract_id.Items.FindByValue(cvalue) == null)
                            {
                                ...
                                fvalue = EvaluateFormula("= Site_inventory_contract.contract_number", itemValue, variables2, evaluator);
                                ...
                                ListItem dupItem = this.site_inventory_contract_id.Items.FindByText(fvalue);
                                ...
                                ListItem newItem = new ListItem(fvalue, cvalue);
                                this.site_inventory_contract_id.Items.Add(newItem);
                ...
                !MiscUtils.SetSelectedDisplayText(this.site_inventory_contract_id, selectedValue))
            {
                ...
                try
                {
                    ...
                    if (rc != null && rc.Length == 1)
                    {
                        ...
                        fvalue = EvaluateFormula("= Site_inventory_contract.contract_number", itemValue, variables2, evaluator2);
        ...
        protected virtual void site_inventory_contract_id_SelectedIndexChanged(object sender, EventArgs args)
        {
            // for the value inserted by quick add button or large list selector, 
            // the value is necessary to be inserted by this event during postback 
            string val = (string)(this.Page.Session[site_inventory_contract_id.ClientID + "_SelectedValue"]);
            string displayText = (string)(this.Page.Session[site_inventory_contract_id.ClientID + "_SelectedDisplayText"]);
            if (!string.IsNullOrEmpty(displayText) && !string.IsNullOrEmpty(val)) {
	            this.site_inventory_contract_id.Items.Add(new ListItem(displayText, val));
	            this.site_inventory_contract_id.SelectedIndex = this.site_inventory_contract_id.Items.Count - 1;
	            this.Page.Session.Remove(site_inventory_contract_id.ClientID + "_SelectedValue");
	            this.Page.Session.Remove(site_inventory_contract_id.ClientID + "_SelectedDisplayText");
            }
...
#region "Helper Properties"
        ...
        public System.Web.UI.WebControls.DropDownList site_inventory_contract_id {
            get {
                return (System.Web.UI.WebControls.DropDownList)BaseClasses.Utils.MiscUtils.FindControlRecursively(this, "site_inventory_contract_id");
            }
        }
    ...
    Site_inventory_contract_historyRecord rec = null;
             ...
             
        {
            ...
            return new Site_inventory_contract_historyRecord();
