...
// Add_Equipment_inventory_finance_note.aspx page.  The Row or RecordControl classes are the 
...
namespace OEMConversion.UI.Controls.Add_Equipment_inventory_finance_note
{
...
// Base class for the Equipment_inventory_finance_noteRecordControl control on the Add_Equipment_inventory_finance_note page.
...
public class BaseEquipment_inventory_finance_noteRecordControl : OEMConversion.UI.BaseApplicationRecordControl
{
        ...
        {      
              ...
              this.equipment_id.TextChanged += equipment_id_TextChanged;
        ...
        public virtual void LoadData()  
        {
            ...
            this.DataSource = (Equipment_inventory_finance_noteRecord)Equipment_inventory_finance_noteRecord.Copy(recList[0], false);
        ...
        public virtual void Setequip_finance_note_datetime()
        {
            ...
            if (this.DataSource != null && this.DataSource.IsCreated) {
              ...
              this.equip_finance_note_datetime.Text = EvaluateFormula("Now()", this.DataSource, @"g");
        ...
        public virtual void Setequip_finance_note_enteredby()
        {
            ...
            // Set the equip_finance_note_enteredby DropDownList on the webpage with value from the
            // DatabaseAYS_OEM%dbo.equipment_inventory_finance_note database record.
            
            // this.DataSource is the DatabaseAYS_OEM%dbo.equipment_inventory_finance_note record retrieved from the database.
            // this.equip_finance_note_enteredby is the ASP:DropDownList on the webpage.
            ...
            else
            {
                ...
                selectedValue = EvaluateFormula("URL(\"equip_finance_note_enteredby\")");
            ...
            this.Populateequip_finance_note_enteredbyDropDownList(selectedValue, 100);              
        ...
        public virtual void Setequipment_id()
        {
            ...
            // Set the equipment_id TextBox on the webpage with value from the
            // DatabaseAYS_OEM%dbo.equipment_inventory_finance_note database record.

            // this.DataSource is the DatabaseAYS_OEM%dbo.equipment_inventory_finance_note record retrieved from the database.
            // this.equipment_id is the ASP:TextBox on the webpage.
                  
            if (this.DataSource != null && this.DataSource.IsCreated) {
                								
                // If the equipment_id is non-NULL, then format the value.
                // The Format method will return the Display Foreign Key As (DFKA) value
               string formattedValue = this.DataSource.equipment_id.ToString();
                            
                this.equipment_id.Text = formattedValue;
              ...
              this.equipment_id.Text = EvaluateFormula("URL(\"equipment_id\")", this.DataSource);
        ...
        {
         // Retrieve the value entered by the user on the equip_finance_note_enteredby ASP:DropDownList, and
        ...
        public virtual void Getequipment_id()
        {
            ...
            // Retrieve the value entered by the user on the equipment_id ASP:TextBox, and
            // save it into the equipment_id field in DataSource DatabaseAYS_OEM%dbo.equipment_inventory_finance_note record.
            
            // Custom validation should be performed in Validate, not here.
                    
            // Save the value to data source
            this.DataSource.Parse(this.equipment_id.Text, Equipment_inventory_finance_noteTable.equipment_id);							
      ...
      // To customize, override this method in Equipment_inventory_finance_noteRecordControl.
         ...
         
        {
            ...
            string recId = this.Page.Request.QueryString["Equipment_inventory_finance_note"];
            ...
            HttpContext.Current.Session["QueryString in Add-Equipment-inventory-finance-note"] = recId;
        ...
        public virtual WhereClause CreateWhereClause_equip_finance_note_enteredbyDropDownList() 
        {
            ...
            if (EvaluateFormula("UserId()", false) != "")filter.AddFilter(new BaseClasses.Data.ColumnValueFilter(BaseClasses.Data.BaseTable.CreateInstance(@"OEMConversion.Business.LoginTable, App_Code").TableDefinition.ColumnList.GetByUniqueName(@"login_.password"), EvaluateFormula("UserId()", false), BaseClasses.Data.BaseFilter.ComparisonOperator.EqualsTo, false));
        ...
        // Fill the equip_finance_note_enteredby list.
        protected virtual void Populateequip_finance_note_enteredbyDropDownList(string selectedValue, int maxItems) 
        {
            		  					                
            this.equip_finance_note_enteredby.Items.Clear();
            
            // 1. Setup the static list items        
            
              // Add the Please Select item.
              this.equip_finance_note_enteredby.Items.Insert(0, new ListItem(this.Page.GetResourceValue("Txt:PleaseSelect", "OEMConversion"), "--PLEASE_SELECT--"));
            		  			
            // 2. Set up the WHERE and the ORDER BY clause by calling the CreateWhereClause_equip_finance_note_enteredbyDropDownList function.
            // It is better to customize the where clause there.
            
                      
            WhereClause wc = CreateWhereClause_equip_finance_note_enteredbyDropDownList();
            ...
            // 3. Read a total of maxItems from the database and insert them into the equip_finance_note_enteredbyDropDownList.
                ...
                do
                {
                    ...
                    foreach (TechniciansRecord itemValue in itemValues) 
                    {
                        ...
                        if (itemValue.technician_idSpecified) 
                        {
                            ...
                            if (counter < maxItems && this.equip_finance_note_enteredby.Items.FindByValue(cvalue) == null)
                            {
                                ...
                                ListItem dupItem = this.equip_finance_note_enteredby.Items.FindByText(fvalue);
                                ...
                                this.equip_finance_note_enteredby.Items.Add(newItem);
        ...
        protected virtual void equip_finance_note_enteredby_SelectedIndexChanged(object sender, EventArgs args)
        {
            ...
            string val = (string)(this.Page.Session[equip_finance_note_enteredby.ClientID + "_SelectedValue"]);
            string displayText = (string)(this.Page.Session[equip_finance_note_enteredby.ClientID + "_SelectedDisplayText"]);
            if (!string.IsNullOrEmpty(displayText) && !string.IsNullOrEmpty(val)) {
	            this.equip_finance_note_enteredby.Items.Add(new ListItem(displayText, val));
	            this.equip_finance_note_enteredby.SelectedIndex = this.equip_finance_note_enteredby.Items.Count - 1;
	            this.Page.Session.Remove(equip_finance_note_enteredby.ClientID + "_SelectedValue");
	            this.Page.Session.Remove(equip_finance_note_enteredby.ClientID + "_SelectedDisplayText");
            }
        ...
        protected virtual void equipment_id_TextChanged(object sender, EventArgs args)
        {
...
#region "Helper Properties"
        ...
        public System.Web.UI.WebControls.DropDownList equip_finance_note_enteredby {
            get {
                return (System.Web.UI.WebControls.DropDownList)BaseClasses.Utils.MiscUtils.FindControlRecursively(this, "equip_finance_note_enteredby");
            }
        }
        ...
        public System.Web.UI.WebControls.TextBox equipment_id {
            get {
                return (System.Web.UI.WebControls.TextBox)BaseClasses.Utils.MiscUtils.FindControlRecursively(this, "equipment_id");
            }
        }
    ...
    Equipment_inventory_finance_noteRecord rec = null;
             ...
             
        {
            ...
            return new Equipment_inventory_finance_noteRecord();
